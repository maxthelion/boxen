...
5. **Subdivision axis rule**: Only subdivide along axes where compartments remain accessible. A divider along axis A creates compartments accessible through faces on OTHER axes. So axis A is valid if ANY face on a different axis is open. For top-open: x and z work (top is on the Y axis, so Y dividers have no perpendicular open face — but X and Z dividers do). For front-open (shelves): x and y work. For multi-open-face boxes: more axes become valid.

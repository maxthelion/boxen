...
import { syncStoreToEngine, getEngine, getEngineVoidTree, ensureEngineInitialized, getEngineFaces, getEngineConfig, getEngineSnapshot, dispatchToEngine, notifyEngineStateChanged } from '../engine';
...
export const useBoxStore = create<BoxState & BoxActions>((set, get) => ({
  ...
  // DEPRECATED: Panels are now generated by engine, use useEnginePanels() hook
    ...
    set((state) => {
      ...
      if (activeOperation === 'subdivide' || activeOperation === 'subdivide-two-panel') {
        ...
        if (voidId && axis && positions && positions.length > 0) {
          ...
          // Notify React that engine state changed
        ...
        if (faceId && offset !== undefined && mode) {
          ...
          // This is simplified - full implementation would match updatePreviewFaceOffset

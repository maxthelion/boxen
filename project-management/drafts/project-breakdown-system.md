...
The following agent prompts would need updating if the messaging and recommendation systems are implemented:

- **`.orchestrator/prompts/backlog-groomer.md`** — Currently writes proposals to `proposals/active/` (file-based). Needs: DB-backed recommendations, backpressure queries, messaging for questions instead of file drops to `human-inbox/`.
- **Other proposer agents** (architect, test-checker, plan-reader) — Same changes as backlog groomer.
- **Curator** — Currently auto-promotes proposals. Needs: demoted to task-creator only, reads from DB recommendation pool instead of file directories.
